...
void cubic_bezier(t_point *points, t_point p1, t_point p2, t_point p3, t_point p4, int step)
{
	...
	while (step)
	{
		points[step - 1].x = pow(1 - t, 3) * p1.x + 3 * t * pow(1 - t, 2) * p2.x + 3 * pow(t, 2) * (1 - t) * p3.x + pow(t, 3) * p4.x;
		points[step - 1].y = pow(1 - t, 3) * p1.y + 3 * t * pow(1 - t, 2) * p2.y + 3 * pow(t, 2) * (1 - t) * p3.y + pow(t, 3) * p4.y;
		step--;
		t += x;
	}
}
